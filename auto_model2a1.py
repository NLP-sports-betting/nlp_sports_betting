...
def auto_knn_metrics( X_train, y_train, X_validate, y_validate,k_range=range(5,20)): #for default k_range should be set to 5
    metrics = []
    ...
    for k in k_range:
        ...
        print(eval_df)
#call is: atm.auto_knn_metrics( X_train, y_train, X_validate, y_validate,k_range)
...
def auto_random_trees(x_train, y_train, x_validate, y_validate, max_depth=range(5,20)):
    ...
    for x in max_depth:
        ...
        #make it
        rf = RandomForestClassifier(random_state=123, max_depth=x) #increases max depth
        #fit it
...
def auto_random_trees_test(x_test, y_test, x_train, y_train, max_depth):
    #make it
    rf = RandomForestClassifier(max_depth, random_state=123) #increases max depth
    #fit it
    rf.fit(x_train, y_train)
        #transform it
        #train_acc = rf.score(x_train, y_train)
...
#call should be: atm.auto_knn_scores(X_train, y_train, X_validate, y_validate, k_range) 

#This function runs through a range of max_depth values and returns the accuracy scores for each max_depth value
def auto_random_forest_scores(x_train, y_train, x_validate, y_validate, max_depth=range(5,21)):
    scores_all = []
...
def auto_random_forest_entropy_scores(x_train, y_train, x_validate, y_validate, max_depth=range(5,21)):
    scores_all = []
